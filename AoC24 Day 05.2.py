...
for pages in invalid_pages:
    ...
    while i < len(pages):
        valid = True
        remaining_keys = pages[i + 1:]
        #print(i, pages[i], remaining_keys)
        for key in remaining_keys:
            key_index = pages.index(key)
            #print(f"Key index {key_index}")
            if key in rules:
            #if key in test_rules:
                #print(f"Key {key} in test_rules")
                if pages[i] in rules[key]:
                #if pages[i] in test_rules[key]:
                    #print(f"Found invalid page {page[i]}")
                    valid = False
                    pages[i], pages[key_index] = pages[key_index], pages[i]
                    break
            #else:
                #print(f"Key {key} not in test_rules")
        if valid:
            i += 1
        else:
            i = 0
        max_runs += 1
        #print(f"Max runs counter: {max_runs}")
        if max_runs > 1000:
            break
    mid_sum += pages[int((len(pages)-1)/2)]
