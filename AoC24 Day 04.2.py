...
def count_mas(word_search):
    a_check = [""]
    for y in range(3, len(word_search) - 3):
        for x in range(3, len(word_search[y]) - 3):
            if word_search[y][x] == "A":
                a_check.append(word_search[y-1][x-1] + word_search[y-1][x+1] + word_search[y+1][x-1] + word_search[y+1][x+1])

    return a_check.count("MMSS") + a_check.count("MSMS") + a_check.count("SMSM") + a_check.count("SSMM")
...
print(count_mas(test_data))
print(f"The number of XMAS in the word search: {count_xmas(data)}")
print(f"The number of X-MAS in the word search: {count_mas(data)}")
