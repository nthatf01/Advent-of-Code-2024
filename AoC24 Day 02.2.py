...
def apply_dampener(report):
    #Remove each level element one at a time, check if that makes a safe report, then repeat
    for i in range(len(report)):
        temp_level = report.pop(i)
        if check_if_safe(report) == True:
            return True
        else:
            report.insert(i, temp_level)
    return False
